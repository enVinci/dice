...
def print_bip39_table(cols, components):
    components_def = {
        "index" : [f'{"Index":<6}', 6],
        "hex"   : [f'{"Hex":<4}', 4],
        "bin"   : [f'{"Binary":<12}', 12],
    ...
    for key in components:
        header += components_def[key][0] + f'| '
        underline_count += components_def[key][1] + 2
    # Print the table header
    for j in range(cols):
        print(header[:-2], end="\t")
    print()
    # Print the table underline
    for j in range(cols):
        print('-' * (underline_count - 2), end="\t")
    print()
    # Print each word
        ...
        for j in range(cols):
            ...
            if index < rows:
                ...
                if 'index' in components:
                    content += f'{index:04}{"":<2}' + f'| '
                if 'hex' in components:
                    content += f'{index:03X}{"":<1}' + f'| '
                if 'bin' in components:
                    content += f'{index:011b}{"":<1}' + f'| '
                if 'word' in components:
                    content += f'{wl[index]:<8}' + f'| '
                print(content[:-2], end="\t")
        print()
...
def main():
    ###Added
    parser = argparse.ArgumentParser('Create a seed by Dice Roll\nExample usage:\necho 123456123456 | python3 rolls.py')
    parser.add_argument("-q", "--qrcode-to-words", default=0, action="count", help="Convert a QR code (its encoded decimal payload) to BIP39 mnemonic. Uses build-in camera by default. (e.g. of 12 word mnemonic payload: 192402220235174306311124037817700641198012901210).")
    parser.add_argument("-w", "--words-to-qrcode", default=False, action="store_true", help="Convert a BIP39 mnemonic to a QR code.")
    parser.add_argument("-t", "--table-bip39", default=False, action="store_true", help="Print the BIP39 mnemonic table.")
    parser.add_argument("-i", "--qrcode-image", default=None, action="store", help="Provide a QR code image file path. Use the 'qrcode.png -s10 -lH' to set qrencode options.")
    args = parser.parse_args()
    if args.qrcode_to_words:
        if args.qrcode_to_words > 1:
            line = input().strip()
        else:
            if args.qrcode_image:
                result = subprocess.run(['zbarimg', '-1', args.qrcode_image], stdout=subprocess.PIPE)
            elif args.qrcode_to_words:
                result = subprocess.run(['zbarcam', '-1'], stdout=subprocess.PIPE)
            line = result.stdout.decode('utf-8').partition('\n')[0]
            prefix = "QR-Code:"
            if not line.startswith(prefix):
                raise Exception('No Qrcode found on the image!')
            line = line[len(prefix):]

        n = 4
        [print(wl[int(line[i:i+n])], end=' ') for i in range(0, len(line), n)]
        print()
        return 0

    if args.words_to_qrcode:
        line = input().strip().split()
        wm = dict(zip(wl, list(range(0, len(wl)))))
        qrcode_payload = [f'{wm[word]:04}' for word in line]
        print(''.join(qrcode_payload))
        print(''.join([f'{wm[word]:03X}' for word in line]))
        print(mnemonic_to_bytes(line))
        if args.qrcode_image:
            subprocess_args = ['qrencode', ''.join(qrcode_payload), '-o']
            subprocess_args.extend(args.qrcode_image.strip().split())
            subprocess.run(subprocess_args, stdout=subprocess.PIPE).stdout.decode('utf-8')
        return 0

    if args.table_bip39:
        print_bip39_table(4, ['index', 'hex', 'bin', 'word'])
        return 0
    ###
